...
class PConv2d(nn.Module):
    def __init__(self, dim, n_div=4, kernel_size=3):
        ...
        super().__init__()
        # 计算参与卷积的通道数 (dim // n_div)
        self.dim_conv3 = dim // n_div
        # 计算保持不变的通道数
        self.dim_untouched = dim - self.dim_conv3
        
        # 定义对部分通道进行卷积的操作 (3x3 conv)
        # 注意：这里 bias=False 是因为通常后面会接 BatchNorm，但在 FasterNetBlock 中具体位置视结构而定
        ...
        # 权重初始化 (可选，根据具体训练框架调整)
        if hasattr(self.partial_conv3, 'weight'):
            nn.init.kaiming_normal_(self.partial_conv3.weight, mode='fan_out', nonlinearity='relu')
    ...
    def forward(self, x):
        ...
        # 2. 对第一部分 (x1) 进行卷积提取空间特征
        x1 = self.partial_conv3(x1)
        
        # 3. 将卷积后的部分 (x1) 与未处理的部分 (x2) 拼接回去
        x = torch.cat((x1, x2), 1)
...
class LKP(nn.Module):
    def __init__(self, dim, lks, sks, groups):
        ...
        self.act = nn.ReLU()
        self.cv2 = PConv2d(in_channels=dim // 2, kernel_size=lks, padding=(lks - 1) // 2, partial_ratio=0.25)
